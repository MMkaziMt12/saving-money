...
import { fetchEmergencyRequestDetails, fetchRelatedNotificationsForRequest, type EmergencyRequestDetail, type RelatedNotificationForRequest } from "@/lib/api/emergencyRequests";
import { createClient } from "@/lib/supabase/client"; // Client-side Supabase

interface RequestDetailClientContentProps {
  requestId: string;
  initialRequestDetails: EmergencyRequestDetail | null;
  initialNotifications: RelatedNotificationForRequest[];
}
...
export function RequestDetailClientContent({ requestId, initialRequestDetails, initialNotifications }: RequestDetailClientContentProps) {
  const router = useRouter();
  const { toast } = useToast(); // Not used, but kept for consistency or future use
  const { user, isLoading: authLoading, isApproved } = useAuth();
  const supabase = createClient(); // Client for client-side fetches
  ...
  const { 
    ...
    initialData: initialRequestDetails,
    enabled: !!requestId && !!user, // Only fetch if requestId and user are available
  });
  ...
  const clientSideLoading = authLoading || (isLoadingRequest && !requestDetails) || (isLoadingNotifications && !notifications);
  const clientSideError = (isRequestError && !requestDetails) ? requestErrorObj : (isNotificationsError && !notifications) ? notificationsErrorObj : null;
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full py-10 text-center px-4">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          {clientSideError.message || "An unknown error occurred."}
        </p>
        <Button onClick={() => {
  ...
  if (!requestDetails) { // This means RLS might have prevented fetch, or request doesn't exist
    return (
      <div className="flex flex-col items-center justify-center h-full py-10 text-center px-4">
        <ListChecks className="h-12 w-12 text-muted-foreground mb-4" />
        <p className="text-muted-foreground mb-4">Emergency request not found or you do not have permission to view it.</p>
  ...
  return (
    <div className="container mx-auto py-8 px-4 md:px-0 max-w-4xl space-y-8">
      <Button onClick={() => router.back()} variant="outline" className="mb-2 print:hidden">
      ...
      <Card className="shadow-xl">
        ...
        <CardContent>
          ...
          ) : isNotificationsError && !notifications ? (
