...
import { createClient } from "@/lib/supabase/client"; // Client for client-side fetches

interface UserDetailClientContentProps {
  userId: string;
  initialUserProfile: Profile | null;
  initialContributions: UserContributionForAdminDetail[];
  initialEmergencyRequests: UserEmergencyRequestForAdminDetail[];
}
...
export function UserDetailClientContent({ 
  userId, 
  initialUserProfile, 
  initialContributions, 
  initialEmergencyRequests 
}: UserDetailClientContentProps) {
  ...
  const { 
    ...
    initialData: initialEmergencyRequests,
    enabled: !!userId && isAdmin,
  });
  ...
  if (authLoading || (!isAdmin && !authLoading && !adminUser)) {
    return (
  ...
  // Combined loading for initial data from hydration
  const combinedIsLoadingInitial = (isLoadingProfile && !userProfile) || 
  ...
  if (combinedIsLoadingInitial && !combinedError) {
    return (
  ...
  if (!userProfile) { // Handle case where userProfile couldn't be fetched (e.g., RLS or not found)
    return (
      <div className="flex flex-col items-center justify-center h-full py-10 text-center px-4">
  ...
  return (
    <div className="container mx-auto py-8 px-4 sm:px-0 max-w-4xl space-y-8">
      <Button onClick={() => router.push("/admin?tab=users")} variant="outline" className="mb-2 print:hidden">
        ...
        <CardContent className="space-y-4">
          <div className="relative">
            ...
            <Input
              ...
              disabled={isLoadingContributions && !!contributions} // Disable if refetching with stale data
            />
          </div>
