...
async function fetchTotalFamilySavingsRPC(): Promise<number> {
  ...
  if (error) {
    ...
    // Log the raw data even on error, if available, for more context
    if (data !== undefined) {
        console.log("RPC 'get_total_family_savings' raw data received (on error):", data);
    }
    throw error;
  }
...
export default function DashboardPage() {
  ...
  const { 
    ...
    error: allEmergencyRequestsError,
    ...
    enabled: !!user, // Assuming all users can see this
    keepPreviousData: true,
  });
  ...
  // Memoized calculations for StatCards
  const userContributionStats = useMemo(() => {
    ...
    const paymentDifferenceMonths = numContributionsMade - monthsSinceJoined;
    ...
    if (paymentDifferenceMonths > 0) {
      pendingAmountVal = 0;
      detailedDescriptionVal = `Paid in advance for ${paymentDifferenceMonths} month${paymentDifferenceMonths > 1 ? 's' : ''}!`;
      statusIconVal = Gift;
      ...
      pendingAmountVal = 0;
      if (monthsSinceJoined === 1 && numContributionsMade === 0) {
    ...
    return { totalPaid, statusText: statusTextVal, pendingAmount: pendingAmountVal, detailedDescription: detailedDescriptionVal, statusIcon: statusIconVal, valueColorClass: valueColorClassVal };
  ...
  // Main loading state for the dashboard
  ...
  // Critical error for user profile or their main contribution status data
     ...
     return (
        <div className="flex flex-col items-center justify-center h-full py-10 text-center">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              {allContributionsForStatusError?.message || "User profile or contribution status could not be loaded."}
            </p>
            <Button 
                onClick={() => {
                    if (!profile && fetchProfile) fetchProfile(user!.id); // AuthContext might have fetchProfile
    ...
    <div className="container mx-auto py-8 px-4 md:px-0">
      ...
      <div className="grid gap-4 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 mb-8">
        ...
        <StatCard
          ...
          description={isLoadingAllContributionsForStatus ? "Fetching..." : (isAllContributionsForStatusError ? allContributionsForStatusError.message : userContributionStats.detailedDescription)}
        ...
        <StatCard
          ...
          icon={TrendingDown}
          description={isLoadingAllEmergencyRequests ? "Fetching..." : (isAllEmergencyRequestsError ? allEmergencyRequestsError.message : "Total amount paid out for approved emergency requests.")}
        ...
        <StatCard
          ...
          icon={Coins}
          description={isLoadingAllEmergencyRequests ? "Fetching..." : (isAllEmergencyRequestsError ? allEmergencyRequestsError.message : "Total amount currently owed back to the fund from approved requests.")}
      ...
      <div className="space-y-8">
        <PaymentHistoryTable 
          ...
          isLoading={isLoadingUserContributions && !userContributionsData?.data} 
          error={isUserContributionsError ? userContributionsError : null}
        ...
        <EmergencyRequestHistoryTable
          ...
          isLoading={isLoadingAllEmergencyRequests && !allEmergencyRequestsData?.data} 
          error={isAllEmergencyRequestsError ? allEmergencyRequestsError : null}
