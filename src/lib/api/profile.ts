...
export async function fetchUserProfileFromServer(userId: string): Promise<Profile | null> {
  if (!userId) {
    console.warn("API: fetchUserProfileFromServer called with no userId.");
    return null;
  }
  console.log(`API: Attempting to fetch profile from server for user: ${userId}`);
  try {
    ...
    if (error) {
      ...
      if (typeof window !== 'undefined') {
        localStorage.removeItem("fft_user_profile"); // Use consistent key
      }
      throw new Error(errorMessage); // Re-throw for TanStack Query or other handlers
    }
